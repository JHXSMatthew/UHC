...
public class Message {
	...
	public Message(){
		...
		}.runTaskTimerAsynchronously(Core.get(), 0, 100);
	...
	public void loadMessages(){
		...
		store.put("wall-fall-time3", ChatColor.RED + "距离PVP保护结束 " + ChatColor.GREEN );
		store.put("wall-fall-time4", ChatColor.RED+" 秒");
		store.put("wall-fall-time1", ChatColor.GRAY + ChatColor.BOLD.toString() +  "== 距离PVP保护结束");
		...
		store.put("player-death-msg1", ChatColor.YELLOW + " 将玩家 " + ChatColor.RESET );
		...
		store.put("spec-msg-prefix", "[观察频道] " +  ChatColor.RESET );
		...
		store.put("player-kill-stack1", ChatColor.YELLOW + " 杀人如麻，势不可挡! 连续击杀 ");
		store.put("player-kill-stack2", ChatColor.YELLOW + "  人.");
		
		store.put("player-kill-buff1", ChatColor.YELLOW + " 连续击杀获得增益效果 速度I 持续2秒.");
		store.put("player-kill-health1", ChatColor.YELLOW + " 连续击杀恢复生命值 "+ ChatColor.GREEN  );
		store.put("player-kill-health2", ChatColor.YELLOW + " 点");
	...
	public String phaseTeamName(int i){
		switch(i){
			case 0:
	...
	public String getSpecMsg(){
		...
		switch (i){
			case 1:
				...
				break;
			case 2:
				msg = ChatColor.GREEN +ChatColor.BOLD.toString() + "== 大键盘 1 键传送至玩家身边 ==";
				break;
			case 3:
